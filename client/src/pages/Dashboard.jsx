...
export default function Dashboard() {
  ...
  const handleUpdate = (updatedLink) => { setLinks(links.map((l) => (l._id === updatedLink.id ? { ...l, ...updatedLink } : l))); };
              ...
              <div key={link._id} className={`bg-[#111118] border rounded-xl p-4 sm:p-5 transition-all ${link.isActive ? "border-[#1E1E2E] hover:border-[#6E56CF]/30" : "border-[#1E1E2E] opacity-50"}`}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
                    <button onClick={() => handleCopy(`http://localhost:5000/${link.shortCode}`)} className="p-1.5 sm:p-2 text-[#888899] hover:text-white hover:bg-[#1E1E2E] rounded-lg transition-colors" title="Copy"><Copy size={14} /></button>
